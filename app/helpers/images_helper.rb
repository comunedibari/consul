module ImagesHelper
  ...
  def render_image_cache(image, version, show_caption = true)
    version = image.persisted? ? version : :original
    render partial: "images/image_cache", locals: { image: image,
                                              version: version,
                                              show_caption: show_caption }
  end

  def render_image_attachment(builder, imageable, image)
    val = request.original_fullpath
    if val.start_with?("/amministrazione/") || val.start_with?("/gestione/")
      classe = "button hollow"
    else
  ...
  def render_image_for_comment(image, version, show_caption = true)
    version = image.persisted? ? version : :original
    render partial: "images/image_for_comment", locals: { image: image,
                                              version: version,
                                              show_caption: show_caption }
  end
  ...
  def get_logo_ente
    @ente_img = Setting.where(key: "ente_logo.image").where(pon_id: User.pon_id).first
  end
