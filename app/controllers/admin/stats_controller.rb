class Admin::StatsController < Admin::BaseController
  ...
  def show
    ...
    @proposal_votes = Vote.by_user_pon.includes(:proposal).where(votable_type: 'Proposal').where(debates: { pon_id: User.pon_id }).count
    ...
    @comment_votes = Vote.by_user_pon.includes(:comment).where(votable_type: 'Comment').where(debates: { pon_id: User.pon_id }).count
