class Admin::Moderation::HiddenUsersController < Admin::BaseController
  has_filters %w{without_confirmed_hide all with_confirmed_hide}, only: :index

  before_action :load_user, only: [:confirm_hide, :restore]

  def index
    @users = User.by_user_pon.only_hidden.send(@current_filter).page(params[:page])
  end
  ...
  private
    ...
    def load_user
      @user = User.by_user_pon.with_hidden.find(params[:id])
    end
