class Valuation::BudgetInvestmentsController < Valuation::BaseController
  ...
  def index
    ...
    @investments = if current_user.valuator? && @budget.present?
                     @budget.by_user_pon.investments.visible_to_valuators.scoped_filter(params_for_current_valuator, @current_filter)
                            .order(cached_votes_up: :desc)
                            .page(params[:page])
                   else
                     Budget::Investment.by_user_pon.none.page(params[:page])
                   end
  end
