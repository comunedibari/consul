class Legislation::DraftVersion < ActiveRecord::Base
  ...
  def editable_by?(user)
    process = Legislation::Process.with_hidden.where(id: legislation_process_id).first
    process.author_id == user.id || user.administrator?
  end
  ...
  def in_moderation
    flags_count = -2 && hidden_at = Time.now.getutc
  end
