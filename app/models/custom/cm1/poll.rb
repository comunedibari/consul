class Poll < ActiveRecord::Base
  ...
  def answerable_by?(user)
    ...
    if user.present? and user.is_social? and access_type == 1
      false
    else
      user.present? && current? && (!geozone_restricted || geozone_ids.include?(user.geozone_id))
    end
  end
  ...
  def vote_has_been_confirmed?(user)
    if user
      poll_questions = Poll::Question.where(poll_id: self.id).where.not(title: "----")
      Poll::Answer.where(question_id: poll_questions.ids, submitted: true, author: user).exists?
      #Poll::Voter.where(poll_id: self.id, user_id: user.id, confirmed: true).exists?
    else
      false
    end
  end
