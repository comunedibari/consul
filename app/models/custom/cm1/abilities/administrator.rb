module Abilities
  class Administrator
    ...
    def initialize(user, params, isBlockedPrivacy)
      ...
      can :load_question, Poll::Question
      can :load_answer, Poll::Question
      can :delunderquestion, Poll::Question
      can :editunderquestion, Poll::Question
      can :update_underquestion, Poll::Question
      ...
      can [:search, :edit, :update, :create, :index, :destroy, :relation, :downloadxlsx], Sector

      can :rem_relation, Sector do |item|
        item.editable_by_admin?(item.id)
      end
