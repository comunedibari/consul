module Flaggable
  ...
  included do
    ...
    scope :all_moderate, -> { where("moderation_entity = 2 or (flags_count > 0 and moderation_entity = 1)").where(hidden_at: nil)}
    ...
    scope :with_ignored_flag, -> {flagged.where.not(ignored_flag_at: nil).where(hidden_at: nil).where(moderation_entity: 1)}
